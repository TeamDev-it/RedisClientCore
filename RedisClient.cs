...
namespace TeamDev.Redis
{
	...
	public class RedisClient : DataAccessProvider, IDisposable
	{
		...
		public int RenewConnectionPeriod { get; set; } = 0;
		...
		public override Socket Connect()
		{
			...
			if (Configuration.DataBaseIndex > 0)
			{
				SendCommand(RedisCommand.SELECT, Configuration.DataBaseIndex.ToString());
				WaitComplete();
			}
		...
		internal Socket GetSocket()
		{
			...
			try
			{
				...
				if (_sockets.ContainsKey(tid))
				{
					Log($"SocketFound --> Connected.. {_sockets[tid].Connected}");
					return _sockets[tid];
				}
				return null;
			}
